...
public class AccessToken {
    ...
    public AccessToken(Context context, @NotNull String mProductId) {
...
//            Util.showAuthToast(mContext, "APIKey is incorrect or does not exist.");
            Log.e("Bob", "Unable to Use Amazon Authorization Manager. APIKey is incorrect or does not exist" +
...
//        Util.getPreferences(context).edit().putString(CODE_VERIFIER, verifier).apply();
    ...
    private AuthorizationListener authListener = new AuthorizationListener() {
        ...
        public void onSuccess(Bundle response) {
            ...
            Log.i("Bob", "Authorization successful");
        ...
        public void onError(AuthError ae) {
            Log.e("Bob", "AuthError during authorization", ae);
        }
        ...
        public void onCancel(Bundle cause) {
            Log.e("Bob", "User cancelled authorization");
        }
    };
     ...
     */
    public void getAccessToken(@NotNull String authCode, @NotNull String
            codeVerifier) {
        ...
        try {
            builder.add(ARG_REDIRECT_URI, mAuthManager.getRedirectUri());
            builder.add(ARG_CLIENT_ID, mAuthManager.getClientId());
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, final IOException e) {
                e.printStackTrace();
                Log.i("Bob", "get token faile: " + e.toString());
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                String s = response.body().string();
                Log.i("Bob", "get token success: " + s);
                final TokenResponse tokenResponse = new Gson().fromJson(s, TokenResponse.class);
                //save our tokens to local shared preferences
                Log.i("Bob", "access token: " + tokenResponse.access_token);
                Log.i("Bob", "refresh token: " + tokenResponse.refresh_token);
