...
public class RefreshToken {
     ...
     */
    public void getAccessToken(@NotNull AmazonAuthorizationManager authorizationManager, @NotNull Context context, @NotNull TokenManager.TokenCallback callback) {
...
//        }
     ...
     */
    public void getRefreshToken(@NotNull AmazonAuthorizationManager authorizationManager , String refreshToken){
        ...
        try {
            ...
            Log.i("Bob","add client id error: " + authError.toString());
        }
...
//        final Handler handler = new Handler(Looper.getMainLooper());
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, final IOException e) {
                e.printStackTrace();
                Log.i("Bob","refresh Token fail: " + e.toString());
            }

            @Override
            public void onResponse(Call call, Response response) throws IOException {
                String s = response.body().string();
                Log.i("Bob","refresh Token success: " + s);

                //get our tokens back
                final TokenManager.TokenResponse tokenResponse = new Gson().fromJson(s, TokenManager.TokenResponse.class);
                Log.i("Bob","access token response: " + tokenResponse.access_token);
                Log.i("Bob","refresh token response: " + tokenResponse.refresh_token);
...
//        SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences preferences = context.getSharedPreferences(Common.TOKEN_PREFERENCE_KEY,Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString(Common.PREF_ACCESS_TOKEN, ACCESS_TOKEN);
        editor.putString(Common.PREF_REFRESH_TOKEN, REFRESH_TOKEN);
        editor.putLong(Common.PREF_TOKEN_EXPIRES, (System.currentTimeMillis() + tokenResponse.expires_in * 1000));
        editor.apply();
